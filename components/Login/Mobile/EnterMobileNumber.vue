<template>
  <div class="text-gray-600">
    <div v-if="!email" class="relative h-full">
      <form v-if="!email" @submit.prevent="requestOtp" class="flex flex-col">
        <div class="flex flex-col space-y-3">
          <span class="text-base font-thin text-center text-gray-600"
          ...
          <input
            ...
            required
            class="w-4/5 px-4 py-2 mx-auto text-base font-light transition duration-300 border-none rounded focus:ring-1 focus:ring-primary-500 ring-1 ring-gray-400 focus:border-transparent focus:outline-none"
          />
        </div>
        <div class="relative w-4/5 mx-auto mt-3">
          <button
            class="w-full py-2 my-5 transition duration-300 rounded-md shadow-md focus:outline-none hover:bg-blue-800"
            :class="loading ? 'bg-gray-500' : 'bg-primary-500 '"
            type="submit"
            :disabled="loading"
          >
            <span
              v-if="!loading"
              class="text-lg font-semibold tracking-wider text-gray-200"
            >
              SEND OTP
            </span>
            <span v-else>
        ...
        <div class="px-12 mt-4 text-center">
          <button
            @click="email = !email"
            class="focus:outline-none text-primary-200 hover:text-primary-500"
          >
            Use Email Address
          </button>
        </div>
      </form>
      <div class="flex flex-row w-full mt-8 lg:mt-5">
        <span class="w-1/2 border-t border-gray-400"></span>
        <span class="mx-3 my-auto -mt-3 font-thin text-gray-400">Or</span>
        <span class="w-1/2 border-t border-gray-400"></span>
      </div>
      <div class="flex flex-row justify-center px-4 py-10">
        <div class="mr-4 rounded cursor-pointer hover:shadow-md">
          <svg
            width="154"
            height="43"
            viewBox="0 0 154 43"
