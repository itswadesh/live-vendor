<template>
  <div class="px-4 py-8 bg-white">
    ...
    <div class="row" style="margin-top: 1%">
      <div class="col-sm">
        <h6 class="pb-2 mb-0">Whiteboard containers (1000 x 600)</h6>
        <div
          id="mywhiteboard"
          style="
            width: 1000px;
...
import ZEGO_WB from '~/gql/channel/zegoWhiteBoard.gql'
...
export default {
  ...
  methods: {
    ...
    // },
    async previousPage() {
      this.zgWbView.scroll(
        (this.zgWbView.getPage() - 2) / this.zgWbView.getPageCount(),
        0
      )
      if (this.zgDocsView) {
    ...
    async nextPage() {
      this.zgWbView.scroll(
        this.zgWbView.getPage() / this.zgWbView.getPageCount(),
        0
      )
      if (this.zgDocsView) {
        const page = this.zgDocsView.getCurrentPage()
        if (page < this.zgDocsView.getPageCount())
          this.zgDocsView.flipPage(page + 1)
      }
      // document.getElementById('curPage').value = this.zgWbView.getPage();
    },
    async undo() {
